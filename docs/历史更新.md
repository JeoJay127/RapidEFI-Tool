...
## RapidEFI-Pro-v5.0.0 (基于OpenCore-v1.0.7) -- 2026-6-29

### 新增功能

1. 新增 3 代及以下老平台支持以及自动配置 EFI 能力

2. 新增异机硬件资料导入功能，支持拖拽或选择硬件报告及 ACPI 表目录，可在另一台电脑上导入目标机器资料并自动生成 EFI。

3. 新增配置项分类展示，引导参数、可选 Kexts、可选设置均按用途分类，并补充对应的参数及功能说明，方便快速查找和选择。

4. 新增自动配置 EFI 个性化设置，支持在输出前调整目标 macOS 版本、SMBIOS、声卡布局 ID、Above 4G Decoding、CPU 类型、平台类型及 SSDT 方案。CPU 类型、平台类型和平台信息默认根据硬件识别结果自动匹配，如遇识别偏差或特殊硬件，也可手动调整，兼顾自动化效率与个性化配置需求。

5. 新增完整的 SSDT 自动定制功能，内置 SSDT 工具链，可根据 ACPI 表选择平台补丁、生成定制 SSDT，并自动合并写入 EFI。

6. 新增不兼容 PCI 设备自动处理功能，可生成设备屏蔽 SSDT，并为需要仿冒的显卡生成对应的 SSDT 仿冒补丁。

7. 新增 Intel 核显接口定制，可按 Framebuffer 配置 con 接口、索引号、Bus ID 与接口类型，并提供常用参考方案。

8. 新增 SMBIOS 设置与详细说明，可查看各机型支持的 macOS 范围、适用显卡方案及特殊注意事项。

9. 新增 Booter Quirks 可选配置，提供多套卡 EB 修复方案，也可手动调整各项 Booter Quirks。

10. 新增 Microsoft Surface 系列驱动与 SSDT、惠普 ACPI Patch，以及 HEDT 服务器平台 SSDT-APIC 推荐补丁。

11. 新增混合架构 CPU 的 `ctrsmt=full` 大小核调度参数。

12. 新增独立的“定制 SSDT”工具与指南入口，提供平台补丁、声卡补丁、显卡仿冒、设备屏蔽及亮度补丁等分类说明与操作参考。

### 功能优化

1. 优化 macOS 版本与 SMBIOS 机型联动，根据目标 macOS 自动筛选并推荐兼容机型；切换 SMBIOS 时也会自动调整到受支持的系统版本。

2. 优化硬件信息自动配置范围，补充 CPU、品牌、主板、核显、内建网卡、存储控制器及蓝牙设备的自动配置。

3. 优化硬件兼容性分析与展示，完善显卡、多显卡、网卡、WiFi、蓝牙、磁盘、NVMe 及存储控制器的兼容性提示。

4. 优化 AMD 平台识别与配置规则，补充主板芯片组、NVMe、核显及 AMD Legacy 平台的兼容性处理。

5. 优化 SSDT 定制流程，支持 Windows、macOS、Linux 平台读取 ACPI 表，并增加平台补丁推荐、批量处理和定制指南。

6. 优化无线网卡与 OCLP 联动，Intel、Broadcom、Atheros WiFi 需要补丁时，自动处理相关驱动、SIP、AMFI、启动参数及内核阻止项；选择高通 WiFi 时自动匹配对应蓝牙驱动。

7. 优化 Kext 驱动依赖联动，选择驱动组合时自动添加需要同时启用的子驱动和配套依赖，减少漏选与重复选择。

8. 优化 Intel 核显高级配置，接口定制与 EDID 注入、HDMI 黑屏及接口修复选项保持联动。

9. 优化声卡与 ACPI 配置，支持选择声卡布局 ID，并增加 HPET IRQ Patch 配置入口。

10. 优化自动配置 EFI 输出流程，增加生成进度与日志展示，输出完成后可直接打开 EFI 目录。

11. 优化历史记录与加工 EFI 流程，减少不同配置之间的数据相互影响，便于继续编辑和重新导出。

12. 优化 Intel 12 代及以上平台 CPU 变频驱动联动，选择 MacPro7,1 机型时自动推荐 CPUFriend 与 CPUFriendDataProvider 配套驱动。
