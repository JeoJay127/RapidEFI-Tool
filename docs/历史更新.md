...
## RapidEFI-v5.0.0 (基于OpenCore-v1.0.7) -- 2026-6-28

### 新增功能

1. 新增异机硬件资料导入功能，支持拖拽或选择硬件报告及 ACPI 表目录，可在另一台电脑上导入目标机器资料并自动生成 EFI。

2. 新增配置项分类展示，引导参数、可选 Kexts、可选设置均按用途分类，并补充对应的参数及功能说明，方便快速查找和选择。

3. 新增自动配置 EFI 个性化设置，支持在输出前调整目标 macOS 版本、SMBIOS、声卡布局 ID、Above 4G Decoding、CPU 类型、平台类型及 SSDT 方案。CPU 类型、平台类型和平台信息默认根据硬件识别结果自动匹配，如遇识别偏差或特殊硬件，也可手动调整，兼顾自动化效率与个性化配置需求。

4. 新增完整的 SSDT 自动定制功能，内置 SSDT 工具链，可根据 ACPI 表选择平台补丁、生成定制 SSDT，并自动合并写入 EFI。

5. 新增不兼容 PCI 设备自动处理功能，可生成设备屏蔽 SSDT，并为需要仿冒的显卡生成对应的 SSDT 仿冒补丁。

6. 新增 Intel 核显接口定制，可按 Framebuffer 配置 con 接口、索引号、Bus ID 与接口类型，并提供常用参考方案。

7. 新增 SMBIOS 设置与详细说明，可查看各机型支持的 macOS 范围、适用显卡方案及特殊注意事项。

8. 新增 Booter Quirks 可选配置，提供多套卡 EB 修复方案，也可手动调整各项 Booter Quirks。

9. 新增 Microsoft Surface 系列驱动与 SSDT、惠普 ACPI Patch，以及 HEDT 服务器平台 SSDT-APIC 推荐补丁。

10. 新增混合架构 CPU 的 `ctrsmt=full` 大小核调度参数。

11. 新增独立的“定制 SSDT”工具与指南入口，提供平台补丁、声卡补丁、显卡仿冒、设备屏蔽及亮度补丁等分类说明与操作参考。
